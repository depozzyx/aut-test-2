module.exports = {
  locales: ['en'],
  defaultLocale: 'en',
  localeDetection: false,
  logBuild: false,
  loadLocaleFrom: (lang, ns) =>
    import(`/.locales/${lang}/${ns}.json`).then((m) => m.default),
  pages: {
    '*': [
      'inputs',
      'error',
      'app-general',
      'modal-message',
      'notifications',
      'routing',
      'datepicker',
      'validation',
      'page-titles',
      'auth',
      'user',
      'agents',
    ],
    '/auth/sign-in': ['error', 'validation', 'inputs'],
    '/auth/forgot-password': ['error', 'validation', 'inputs'],
    '/auth/reset-password': ['error', 'validation', 'inputs'],
    '/cabinet/dashboard/active-campaigns': [
      'user',
      'routing',
      'campaigns',
      'agents',
      'validation',
      'campaign-statistics',
    ],
    '/cabinet/campaigns/list': ['user', 'routing', 'campaigns', 'validation'],
    '/cabinet/campaigns/analytics': ['user', 'routing', 'campaigns'],
    '/cabinet/campaigns/view/[campaignId]': [
      'routing',
      'campaigns',
      'leads-list',
      'page-titles',
    ],
    '/cabinet/dashboard/active-agents': [
      'user',
      'routing',
      'agents',
      'campaigns',
      'campaign-statistics',
      'leads-list',
    ],
    '/cabinet/dashboard/call-statistics': [
      'user',
      'routing',
      'agents',
      'campaigns',
      'campaign-statistics',
    ],
    '/cabinet/agents/list': ['user', 'routing', 'agents', 'campaigns'],
    '/cabinet/agents/groups': ['user', 'routing', 'agents', 'campaigns'],
    '/cabinet/agents/create': ['user', 'routing', 'agents'],
    '/cabinet/agents/analytics': ['user', 'routing', 'agents'],
    '/cabinet/leads/list': ['user', 'routing', 'validation', 'leads-list', 'campaigns'],
    '/cabinet/leads': ['user', 'routing', 'validation', 'leads-list'],
    '/cabinet/leads/import': [
      'user',
      'routing',
      'validation',
      'import-leads',
      'leads-list',
    ],
    '/cabinet/leads/import-instructions': [
      'user',
      'routing',
      'validation',
      'import-leads',
    ],
    '/cabinet/agents/calls': ['user', 'routing', 'calls'],
    '/cabinet/managers/list': ['user', 'routing', 'managers'],
    '/cabinet/managers/create': ['user', 'routing', 'managers'],
    '/cabinet/reports/cdr': ['user', 'routing', 'calls-list'],
    '/cabinet/reports/campaign-statistics': ['user', 'routing', 'campaign-statistics'],
    '/cabinet/settings/account-management': ['user', 'routing', 'settings', 'validation'],
    '/cabinet/settings/activity-log/general': [
      'user',
      'routing',
      'activity-log',
      'validation',
    ],
    '/cabinet/settings/activity-log/campaign': [
      'user',
      'routing',
      'validation',
      'activity-log',
    ],
    '/cabinet/settings/admins-list': ['user', 'routing', 'admins', 'validation'],
    '/cabinet/settings/api-key-management': ['user', 'routing', 'api-key', 'validation'],
    '/cabinet/settings/campaigns-management': ['settings', 'routing'],
    '/cabinet/settings/leads-management': ['settings', 'routing'],
  },
}
