...
export const Label = styled.label<{ checked: boolean; disabled?: boolean }>((props) => {
  ...
  return css`
    ...
    width: 52px;
    height: 32px;
    background-color: ${checked ? palette.main2 : palette.main14};
    border-radius: 100px;
    ...
    ${disabled &&
    css`
      background-color: ${checked ? palette.main14 : palette.base};
    `}
  `
})
...
export const Input = styled.input`
  ...
  &:focus + ${ToggleButton} {
    box-shadow: 0 0 0px 8px
      ${({ theme, checked }) =>
        checked
          ? hexToRGBA(theme.palette.base4, 0.25)
          : hexToRGBA(theme.palette.main22, 0.15)};
  }
  ...
  :disabled {
    ...
    + ${ToggleButton} {
      background-color: ${({ theme, checked }) =>
        checked ? theme.palette.base : theme.palette.main14};
    }
  }
`
